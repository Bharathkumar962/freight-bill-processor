"""
Graph layer — models relationships between:
  Carrier → Contract → Lane → Shipment → BOL

Built once at startup from seed data and queried by the agent.
"""
import json
...
def load_graph(seed_path: str = "seed_data_logistics.json"):
    """Parse seed JSON and build the directed graph."""
    global G, _seed_data
...
def find_carrier_by_name(name: str) -> dict | None:
...
def get_contracts_for_carrier(carrier_id: str) -> list[dict]:
    contracts = []
...
def get_active_contracts_for_lane(carrier_id: str, lane: str, bill_date: str) -> list[dict]:
    """
    Return all contracts for a carrier that:
      - cover the requested lane
      - are active on bill_date
    This deliberately returns multiple results when contracts overlap — the agent
    must then decide which to apply (newest effective_date wins by default).
    """
    ...
    for contract in get_contracts_for_carrier(carrier_id):
        ...
        # Check if this contract covers the lane
    ...
    # Sort by effective_date descending — newest contract is preferred
...
def get_bols_for_shipment(shipment_id: str) -> list[dict]:
    bols = []
...
def get_all_shipments_for_contract(contract_id: str) -> list[dict]:
    shipments = []
